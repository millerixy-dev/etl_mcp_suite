...
- `add_paragraph` -> parse interpreter from the body's leading shebang, fail-closed on a missing shebang, validate the parsed interpreter against the allowlist BEFORE network, apply the write-safety gate, POST the body verbatim -> return `notebook_id`, `paragraph_id`, `title`, parsed `interpreter`.
0.10.1 selects the interpreter per-paragraph via a shebang prefix in the paragraph text (e.g. `%spark.sql\nSELECT ...`). The caller (the agent) owns the shebang and therefore the interpreter declaration; the tool does NOT inject or modify a shebang and sends the body verbatim. The service parses the interpreter from the body's first-line shebang (`%<interpreter>`), fails closed with `INVALID_INPUT` when no shebang is present (the notebook's `defaultInterpreterGroup` is unknown to this process and cannot be safely gated), and matches the parsed interpreter against `allowed_interpreters` before applying the write-safety gate. This makes the body the single source of truth for the interpreter, so there is no input parameter that can disagree with the shebang and bypass a gate.
